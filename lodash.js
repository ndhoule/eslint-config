module.exports = {
  plugins: ["lodash"],
  // lodash/recommended has some questionable and very opinionated defaults which largely try to
  // steer you toward using Lodash over e.g. native methods, and often comes into conflict with
  // TypeScript (e.g. for type refinement).
  rules: {
    ...
    // I don't really understand why Lodash has chosen to go against the grain when it comes to
    // function composition.
    "lodash/consistent-compose": ["error", "compose"],
    "lodash/identity-shorthand": ["error", "always"],
    "lodash/prefer-filter": ["error", 3],
    // This rule is pretty dumb, honestly; it forces the use of Lodash where it's not particularly
    // useful or any more clear.
    ...
    // These rules are useful in JavaScript, but are made redundant by TypeScript.
    "lodash/no-extra-args": "error",
    "lodash/no-unbound-this": "error",
    "lodash/collection-return": "error",

    // The use of matching/property shorthand encourages code that is not easily typed. While
    // acutely problematic in TypeScript code (where it results in lots of `any` types), it results
    // in sloppy code in vanilla JavaScript, too.
